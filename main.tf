locals {
  nginx_deployments = { for k1, v1 in var.nginx_deployments : k1 => { auto_scale_profile = v1.auto_scale_profile, automatic_upgrade_channel = v1.automatic_upgrade_channel, capacity = v1.capacity, email = v1.email, frontend_private = v1.frontend_private, frontend_public = v1.frontend_public, identity = v1.identity, location = v1.location, name = v1.name, network_interface = v1.network_interface, resource_group_name = v1.resource_group_name, sku = v1.sku, tags = v1.tags, web_application_firewall = v1.web_application_firewall } }
